...
class ClassificationDataset(Dataset):
    ...
    def __len__(self):
        return self.df.shape[0]
    def __getitem__(self, idx):
        # Shuffle label names for augmentation
        ...
        # Formatting
        row_dict = self.df.iloc[idx].drop(labels="label").to_dict()
        row_dict["LIST"] = shuffled_class_names
        prompt_part = sampled_prompt.format(**row_dict)

        # Final text
        full_text = prompt_part + self.df["label"].iloc[idx] + self.tokenizer.eos_token
        # Manual implementation of completion-only loss
        tokenized = self.tokenizer(
            full_text,
            truncation=True,
        ...
        input_ids = tokenized["input_ids"]
        offsets = tokenized["offset_mapping"]
        prompt_end_char = len(prompt_part)
        labels = [
            -100 if start < prompt_end_char else token_id
            for (start, _), token_id in zip(offsets, input_ids)
        ]
